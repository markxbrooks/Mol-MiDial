...
try:
    ...
    print(f"API_UNSPECIFIED: {getattr(rtmidi, 'API_UNSPECIFIED', 'NO API_UNSPECIFIED')}")
    print(f"Available functions: {[attr for attr in dir(rtmidi) if not attr.startswith('_')]}")
        ...
        with mido.open_input(port_name) as inport:
            ...
            for msg in inport:
                if msg.type == 'control_change':
                    print(f"{msg.time:8.3f} | Channel {msg.channel+1:2d} | "
                          f"Controller {msg.control:3d} | Value {msg.value:3d}")
                elif msg.type == 'note_on':
                    print(f"{msg.time:8.3f} | Note ON  | Channel {msg.channel+1:2d} | "
                          f"Note {msg.note:3d} | Velocity {msg.velocity:3d}")
                elif msg.type == 'note_off':
                    print(f"{msg.time:8.3f} | Note OFF | Channel {msg.channel+1:2d} | "
                          f"Note {msg.note:3d} | Velocity {msg.velocity:3d}")
...
def test_elmo_midi_controller():
    ...
    try:
        ...
        for name, info in mappings.items():
            print(f"  {name}: Control {info['control_number']} -> {info['target_function']}")
...
if __name__ == "__main__":
    ...
    try:
        port_index = input(f"Enter port number (0-{len(ports)-1}) or 'q' to quit: ").strip()
